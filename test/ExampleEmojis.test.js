const ExampleEmojis = artifacts.require("./ExampleEmojis")
...
contract('ExampleEmojis', ([deployer, user]) => {

    const NAME = 'Example Emojis'
    const SYMBOL = 'EE'
    const COST = 0
    const MAX_SUPPLY = 50
    ...
    describe('Deployment', () => {
        ...
        beforeEach(async () => {
            ...
            exampleEmojis = await ExampleEmojis.new(
                NAME,
                SYMBOL,
                COST,
                MAX_SUPPLY,
        ...
        it('Returns the contract name', async () => {
            result = await exampleEmojis.name()
            result.should.equal(NAME)
        })

        it('Returns the symbol', async () => {
            result = await exampleEmojis.symbol()
            result.should.equal(SYMBOL)
        })

        it('Returns the cost to mint', async () => {
            result = await exampleEmojis.cost()
            result.toString().should.equal(COST.toString())
        })

        it('Returns the max supply', async () => {
            result = await exampleEmojis.maxSupply()
            result.toString().should.equal(MAX_SUPPLY.toString())
        })

        it('Returns the max mint amount', async () => {
            result = await exampleEmojis.maxMintAmount()
            result.toString().should.equal('1')
        })

        it('Returns the time deployed', async () => {
            result = await exampleEmojis.timeDeployed()
        ...
        it('Returns the amount of seconds from deployment to wait until minting', async () => {
            ...
            result = await exampleEmojis.allowMintingAfter()
        ...
        it('Returns how many seconds left until minting allowed', async () => {
            let buffer = 8
            let target = Number(milliseconds.toString().slice(0, 3))
            result = await exampleEmojis.getSecondsUntilMinting()
            result = Number(result)
        ...
        it('Returns current pause state', async () => {
            result = await exampleEmojis.isPaused()
            result.toString().should.equal('false')
        })

        it('Returns current reveal state', async () => {
            result = await exampleEmojis.isRevealed()
    ...
    describe('Minting', async () => {
        describe('Success', async () => {
            ...
            beforeEach(async () => {
                ...
                result = await exampleEmojis.mint(1, { from: user, value: web3.utils.toWei('0', 'ether') })
            })
            ...
            it('Updates the total supply', async () => {
                result = await exampleEmojis.totalSupply()
                result.toString().should.equal('1')
            })

            it('Returns IPFS URI', async () => {
                result = await exampleEmojis.tokenURI(1)
                result.should.equal(`${IPFS_IMAGE_METADATA_URI}1.json`)
            })

            it('Returns how many a minter owns', async () => {
                result = await exampleEmojis.balanceOf(user)
                result.toString().should.equal('1')
            })

            it('Returns the IDs of minted NFTs', async () => {
                result = await exampleEmojis.walletOfOwner(user)
        ...
        describe('Failure', async () => {
            ...
            beforeEach(async () => {
                ...
                const NFT_MINT_DATE = new Date("April 6, 2022 21:00:00").getTime().toString().slice(0, 10)
            ...
            it('Attempt to mint before mint date', async () => {
                await exampleEmojis.mint(1, { from: user, value: web3.utils.toWei('0', 'ether') }).should.be.rejectedWith(EVM_REVERT)
            })
        })
    })
    ...
    describe('Updating Contract State', async () => {
        describe('Success', async () => {
            ...
            it('Sets the cost', async () => {
                ...
                await exampleEmojis.setCost(cost, { from: deployer })
                result = await exampleEmojis.cost()
            ...
            it('Sets the pause state', async () => {
                ...
                await exampleEmojis.setIsPaused(isPaused, { from: deployer })
                result = await exampleEmojis.isPaused()
            ...
            it('Sets the reveal state', async () => {
                ...
                await exampleEmojis.setIsRevealed(isRevealed, { from: deployer })
                result = await exampleEmojis.isRevealed()
            ...
            it('Sets the max batch mint amount', async () => {
                ...
                await exampleEmojis.setmaxMintAmount(5, { from: deployer })
                result = await exampleEmojis.maxMintAmount()
            ...
            it('Sets the IPFS not revealed URI', async () => {
                ...
                await exampleEmojis.setNotRevealedURI(uri, { from: deployer })
                result = await exampleEmojis.notRevealedUri()
            ...
            it('Sets the base extension', async () => {
                ...
                await exampleEmojis.setBaseExtension('.example', { from: deployer })
                result = await exampleEmojis.baseExtension()
