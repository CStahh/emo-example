...
// Import ABI + Config
import ExampleEmojis from '../abis/ExampleEmojis.json'
import config from '../config.json'

function App() {
	const [web3, setWeb3] = useState(null)
	const [exampleEmojis, setExampleEmojis] = useState(null)
	...
	const loadBlockchainData = async (_web3, _account, _networkId) => {
		...
		try {
			const exampleEmojis = new _web3.eth.Contract(ExampleEmojis.abi, ExampleEmojis.networks[_networkId].address)
			setExampleEmojis(exampleEmojis)

			const maxSupply = await exampleEmojis.methods.maxSupply().call()
			const totalSupply = await exampleEmojis.methods.totalSupply().call()
			setSupplyAvailable(maxSupply - totalSupply)

			const allowMintingAfter = await exampleEmojis.methods.allowMintingAfter().call()
			const timeDeployed = await exampleEmojis.methods.timeDeployed().call()
			setRevealTime((Number(timeDeployed) + Number(allowMintingAfter)).toString() + '000')

			if (_account) {
				const ownerOf = await exampleEmojis.methods.walletOfOwner(_account).call()
				setOwnerOf(ownerOf)
				console.log(ownerOf)
			} else {
				setOwnerOf([])
	...
	const mintNFTHandler = async () => {
		...
		// Mint NFT
		if (exampleEmojis && account) {
			setIsMinting(true)
			setIsError(false)

			await exampleEmojis.methods.mint(1).send({ from: account, value: 10000000000000000 })
				.on('confirmation', async () => {
					const maxSupply = await exampleEmojis.methods.maxSupply().call()
					const totalSupply = await exampleEmojis.methods.totalSupply().call()
					setSupplyAvailable(maxSupply - totalSupply)

					const ownerOf = await exampleEmojis.methods.walletOfOwner(account).call()
					setOwnerOf(ownerOf)
				})
				.on('error', (error) => {
					window.alert(error)
				...
				<section id='about' className='about'>
								...
								<div>
									...
									{ownerOf.length > 0 &&
										<p><small>View your NFT on
											<a
												href={`${openseaURL}/assets/${exampleEmojis._address}/${ownerOf[0]}`}
												target='_blank'
					...
					<Row style={{ marginTop: "100px" }}>
						<Col>
							{exampleEmojis &&
								<a
									href={`${explorerURL}/address/${exampleEmojis._address}`}
									target='_blank'
									className='text-center'>
									{exampleEmojis._address}
								</a>
							}
						</Col>
					</Row>
